...
char **add_end_line(char **map, int c2, int c3, size s_map)
{
...
char **filled_line(char **map, int c2, int c3, size s_map)
{
...
char **map_error(char **map, size s_map)
{
...
void create_map(size s_map)
{
    ...
    for (int c2 = 0; c2 != s_map.y; c2++)
        for (int c3 = 0; c3 != s_map.x + 1; c3++) {
            add_end_line(map, c2, c3, s_map);
            filled_line(map, c2, c3, s_map);
        }
    map = map_error(map, s_map);
    change_map(map, s_map);
}
